    ...
    const columns = useMemo(() => [
        ...
        },
        {
            ...
            cell: ({ row }: any) => (
                <div className="flex flex-col">
                    <span className="font-semibold text-sm">{row.original.customer?.nama || 'Umum'}</span>
                    <span className="text-[10px] text-muted-foreground font-medium">Retail Customer</span>
                </div>
            ),
        },
        {
        ...
        },
        {
            ...
            cell: ({ row }: any) => {
                ...
                let colorClass = 'bg-yellow-500/10 text-yellow-600 border-yellow-500/20';
                
                if (status === 'Sudah Dibayar') {
                    colorClass = 'bg-green-500/10 text-green-600 border-green-500/20';
                } else if (status === 'Belum Dibayar Sepenuhnya') {
                    colorClass = 'bg-blue-500/10 text-blue-600 border-blue-500/20';
                }

                return (
                    <div className={`inline-flex items-center rounded-full border px-3 py-1 text-[10px] font-black uppercase tracking-wider shadow-sm ${colorClass}`}>
                        {status}
                    </div>
                );
            },
        },
        {
            ...
            cell: ({ row }: any) => (
                <div className="flex flex-col">
                    <span className="text-xs font-semibold">{new Date(row.getValue('created_at')).toLocaleDateString('id-ID', { day: '2-digit', month: 'short' })}</span>
                    <span className="text-[10px] text-muted-foreground">{new Date(row.getValue('created_at')).toLocaleTimeString('id-ID', { hour: '2-digit', minute: '2-digit' })}</span>
                </div>
            ),
        },
        {
    ...
    return (
            ...
            header={
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Kelola transaksi kasir anda</p>
                    </div>
                    <Link href={route('orders.create')}>
                        <Button className="h-11 px-6 font-black uppercase tracking-tighter shadow-lg shadow-primary/25 rounded-xl">
                            <Plus className="mr-2 h-5 w-5" /> Buat Transaksi
                        </Button>
                    </Link>
                </div>
            }
        >
                                            ...
                                            <TableRow>
                                                <TableCell colSpan={columns.length} className="h-48 text-center">
                                                    <div className="flex flex-col items-center justify-center space-y-2 opacity-40">
                                                        ...
                                                        <p className="text-sm font-bold uppercase tracking-widest">Tidak ada data transaksi</p>
                                                    </div>
                                                </TableCell>
                                            </TableRow>
                                        )}
            ...
            <DeleteConfirmModal 
                ...
                description="Menghapus data penjualan akan berpengaruh pada laporan stok dan pendapatan."
            />
        </AuthenticatedLayout>
    );
}
