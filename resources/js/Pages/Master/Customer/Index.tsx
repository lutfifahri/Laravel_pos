...
export default function CustomerIndex({ customers }: Props) {
        ...
        },
        {
            ...
            cell: ({ row }: any) => (
                <div className="flex items-center gap-3">
                    ...
                    <span className="font-bold tracking-tight">{row.getValue('nama')}</span>
                </div>
            ),
        },
        {
            ...
            cell: ({ row }: any) => (
                <div className="flex items-center gap-2 text-muted-foreground">
                    ...
                    <span className="text-xs font-semibold">{row.getValue('phone') || '-'}</span>
                </div>
            ),
        },
        {
    ...
    const handleConfirmDelete = () => {
        ...
        router.delete(route('master.customers.destroy', customerToDelete), {
            onSuccess: () => {
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        if (editingCustomer) {
            put(route('master.customers.update', editingCustomer.id), {
                onSuccess: () => {
            ...
            header={
                <div className="flex items-center justify-between">
                    <div>
                        ...
                        <p className="text-xs text-muted-foreground font-medium uppercase tracking-widest">Kelola data pelanggan setia anda</p>
                    </div>
                    <Button 
                        onClick={openCreateModal}
                        className="h-11 px-6 font-black uppercase tracking-tighter shadow-lg shadow-primary/25 rounded-xl"
                    >
                        <Plus className="mr-2 h-5 w-5" /> Customer Baru
                    </Button>
                </div>
            }
        >
                ...
                <DialogContent className="sm:max-w-[425px] rounded-2xl border-none shadow-2xl">
                    <DialogHeader>
                        <DialogTitle className="text-xl font-black uppercase tracking-tight">
                            {editingCustomer ? 'Edit Customer' : 'Customer Baru'}
                        </DialogTitle>
                        <DialogDescription>
                            Isi informasi pelanggan anda di bawah ini.
                        </DialogDescription>
                    </DialogHeader>
                    <form onSubmit={handleSubmit}>
                        <div className="grid gap-5 py-6">
                            <div className="grid gap-2">
                                <Label htmlFor="nama" className="text-[10px] uppercase font-black text-muted-foreground">Nama Lengkap</Label>
                                <Input
                                    id="nama"
                                    value={data.nama}
                            ...
                            <div className="grid gap-2">
                                <Label htmlFor="phone" className="text-[10px] uppercase font-black text-muted-foreground">No. Telepon / WhatsApp</Label>
                                <Input
                                    id="phone"
                                    value={data.phone}
            ...
            <DeleteConfirmModal 
                ...
                description="Menghapus customer akan menghilangkan data riwayat mereka namun data transaksi tetap tersimpan."
            />
        </AuthenticatedLayout>
    );
}
